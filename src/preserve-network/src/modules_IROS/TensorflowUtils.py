...
def get_constant(weights, name):
    return tf.constant(weights, name=name)
...
def conv2d_basic(x, W, bias):
    ...
    #conv = tf.nn.conv2d(x, W, strides=[1, 1, 1, 1], padding="SAME", data_format="NCHW")
    #return tf.nn.bias_add(conv, bias, data_format="NCHW")

def conv2d_strided(x, W, b):
    conv = tf.nn.conv2d(x, W, strides=[1, 2, 2, 1], padding="SAME")
    return tf.nn.bias_add(conv, b)
    #conv = tf.nn.conv2d(x, W, strides=[1, 2, 2, 1], padding="SAME", data_format="NCHW")
    #return tf.nn.bias_add(conv, b, data_format="NCHW")
...
def conv2d_transpose_strided(x, W, b, output_shape=None, stride = 2):
    ...
    return tf.nn.bias_add(conv, b)
    #conv = tf.nn.conv2d_transpose(x, W, output_shape, strides=[1, stride, stride, 1], padding="SAME",data_format="NCHW")
    #return tf.nn.bias_add(conv, b, data_format="NCHW")
...
def max_pool_2x2(x):
    ...
    #return tf.nn.max_pool(x, ksize=[1, 2, 2, 1], strides=[1, 2, 2, 1], padding="SAME", data_format="NCHW")
...
def bottleneck_unit(x, out_chan1, out_chan2, down_stride=False, up_stride=False, name=None):
    ...
    def conv_transpose(tensor, out_channel, shape, strides, name=None):
        ...
        #return tf.nn.conv2d_transpose(x, kernel, output_shape=out_shape, strides=[1, strides, strides, 1],padding='SAME', data_format="NCHW", name='conv_transpose')
    ...
    def conv(tensor, out_chans, shape, strides, name=None):
        ...
        #return tf.nn.conv2d(x, kernel, strides=[1, strides, strides, 1], padding='SAME', data_format="NCHW", name='conv')
...
def get_bb_npz(file_path, point):

    bb = []

    for f in file_path[point: point + 1]:
        bb_unzip = np.load(f)
        bb.append(bb_unzip['a'])
...
def get_img(file_path, point):

    img = []

    im = ndimage.imread(file_path[point])
    img.append(im)
