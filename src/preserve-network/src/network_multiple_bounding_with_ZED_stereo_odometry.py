...
winW = [10]
winH = [10]
CROP_SHAPE = [((winW[0]), (winH[0]), 3)]
CROP_SIZE = [reduce(mul, CROP_SHAPE[0])]
for i in xrange(4):
    winW.append(int(winW[i]*RATE_WINDOW))
    winH.append(int(winH[i]*RATE_WINDOW))
    CROP_SHAPE.append( ((winW[i+1]), (winH[i+1]) , 3) )
    CROP_SIZE.append(reduce(mul, CROP_SHAPE[i+1]))
...
cropoutBUFF_SIZE_1 = CROP_SIZE[0] * CROP_BUFF_NUM_ELEMENT

IMGBUFF_SIZE_2 = IMG_SIZE_2 * BUFF_NUM_ELEMENT
cropoutBUFF_SIZE_2 = CROP_SIZE[1] * CROP_BUFF_NUM_ELEMENT

IMGBUFF_SIZE_3 = IMG_SIZE_3 * BUFF_NUM_ELEMENT
cropoutBUFF_SIZE_3 = CROP_SIZE[2] * CROP_BUFF_NUM_ELEMENT

IMGBUFF_SIZE_4 = IMG_SIZE_4 * BUFF_NUM_ELEMENT
cropoutBUFF_SIZE_4 = CROP_SIZE[3] * CROP_BUFF_NUM_ELEMENT
...
win = tf.placeholder(tf.int32, name="window_size")
threshold = tf.placeholder(tf.float64, name="threshold")
nms_boxes, bb_xy_of_maxScore = FCN_NHWC.inference(image, keep_probability, win, threshold)
...
def img_thread_func():
    while network_thread_running or img_buffer_1.nb_data > 0:
        if img_buffer_1.nb_data > 0:
            ...
            feed_dict = {image: imgs_reshape, keep_probability: 1.0, win: winW[0], threshold: THRESHOLD[0]}
...
def img_thread_func2():
    while network_thread_running2 or (img_buffer_2.nb_data > 0):
        if img_buffer_2.nb_data > 0:
            ...
            feed_dict = {image: imgs_reshape, keep_probability: 1.0, win: winW[1], threshold: THRESHOLD[1]}
...
def img_thread_func3():
    while network_thread_running3 or (img_buffer_3.nb_data > 0):
        if img_buffer_3.nb_data > 0:
            ...
            feed_dict = {image: imgs_reshape, keep_probability: 1.0, win: winW[2], threshold: THRESHOLD[2]}
...
def img_thread_func4():
    while network_thread_running4 or (img_buffer_4.nb_data > 0):
        if img_buffer_4.nb_data > 0:
            ...
            feed_dict = {image: imgs_reshape, keep_probability: 1.0, win: winW[3], threshold: THRESHOLD[3]}
...
def processing_func():
    while processing_thread_running or (NMS_buffer_1.nb_data > 0
        ...
        if (NMS_buffer_1.nb_data > 0 and NMS_buffer_2.nb_data > 0 and NMS_buffer_3.nb_data > 0 and NMS_buffer_4.nb_data > 0 and img_buffer_origin.nb_data > 0 and img_buffer_origin_right.nb_data > 0):
            ...
            if num_boxes_4 > 0:
                dim_5 = (int((float(CAM_HEIGHT) / float(IMG_HEIGHT_4)) * winH[3]),
                       int((float(CAM_WIDTH) / float(IMG_WIDTH_4)) * winW[3]))
                with cropout_buffer_lock_4:
                    for i in xrange(num_boxes_4):
                        if cropout_buffer_4.nb_data == 0:
                            raise Exception("cropout buffer4 has no entry")
                        crop_outs = cropout_buffer_4.get(CROP_SIZE[3])
                        cropout_buffer_4.pop(CROP_SIZE[3])
                        crop_xy = cropout_buffer_4.get(2)
                        cropout_buffer_4.pop(2)

                        crop_outs = crop_outs.reshape((crop_outs.size / CROP_SIZE[3],) + CROP_SHAPE[3])
            ...
            if num_boxes_3 > 0:
                dim_5 = (int((float(CAM_HEIGHT) / float(IMG_HEIGHT_3)) * winH[2]),
                       int((float(CAM_WIDTH) / float(IMG_WIDTH_3)) * winW[2]))

                with cropout_buffer_lock_3:
                    for i in xrange(num_boxes_3):
                        if cropout_buffer_3.nb_data == 0:
                            raise Exception("cropout buffer3 has no entry")
                        crop_outs = cropout_buffer_3.get(CROP_SIZE[2])
                        cropout_buffer_3.pop(CROP_SIZE[2])
                        crop_xy = cropout_buffer_3.get(2)
                        cropout_buffer_3.pop(2)

                        crop_outs = crop_outs.reshape((crop_outs.size / CROP_SIZE[2],) + CROP_SHAPE[2])
            ...
            if num_boxes_2 > 0:
                dim_5 = (int((float(CAM_HEIGHT) / float(IMG_HEIGHT_2)) * (winH[1])),
                       int((float(CAM_WIDTH) / float(IMG_WIDTH_2)) * (winW[1])))

                with cropout_buffer_lock_2:
                    for i in xrange(num_boxes_2):
                        if cropout_buffer_2.nb_data == 0:
                            raise Exception("cropout buffer2 has no entry")
                        crop_outs = cropout_buffer_2.get(CROP_SIZE[1])
                        cropout_buffer_2.pop(CROP_SIZE[1])
                        crop_xy = cropout_buffer_2.get(2)
                        cropout_buffer_2.pop(2)

                        crop_outs = crop_outs.reshape((crop_outs.size / CROP_SIZE[1],) + CROP_SHAPE[1])
            ...
            if num_boxes > 0:
                dim_5 = (int((float(CAM_HEIGHT)/float(IMG_HEIGHT_1))*(winH[0])),int((float(CAM_WIDTH)/float(IMG_WIDTH_1))*(winW[0])))

                with cropout_buffer_lock_1:
                    for i in xrange(num_boxes):
                        if cropout_buffer_1.nb_data == 0:
                            raise Exception("cropout buffer has no entry")
                        crop_outs = cropout_buffer_1.get(CROP_SIZE[0])
                        cropout_buffer_1.pop(CROP_SIZE[0])
                        crop_xy = cropout_buffer_1.get(2)
                        cropout_buffer_1.pop(2)

                        crop_outs = crop_outs.reshape((crop_outs.size / CROP_SIZE[0],) + CROP_SHAPE[0])
    ...
    while publish_thread_running:
        if RESULT_buffer.nb_data > 0 and  RESULT_buffer_right.nb_data > 0:
            ...
            try:
                ...
                NUM_PUBLISH += 1
                if NUM_PUBLISH == 1:
                    TIME_FIRST_PUBLISH = TIME_FIRST_PUBLISH + timeit.default_timer()
...
NETWORK_SHUTDOWN_TIME = timeit.default_timer()
...
print("Thread Termination completed.")
print "FPS: ", NUM_PUBLISH/(NETWORK_SHUTDOWN_TIME - TIME_FIRST_PUBLISH)
print "# of publish: ", NUM_PUBLISH
print "Time when the first result was published: ", TIME_FIRST_PUBLISH
print "Network Shutdown time: ", NETWORK_SHUTDOWN_TIME
