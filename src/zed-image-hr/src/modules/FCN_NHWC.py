...
tf.flags.DEFINE_string("logs_dir", "./", "path to logs directory")
...
def inference(image, keep_prob):
    ...
    with tf.variable_scope("inference"):
        ...
        if os.path.exists(FLAGS.logs_dir + FLAGS.weight):
    ...
    with tf.name_scope('Sliding'):
        kernel = tf.ones(shape=[10, 10, 1, 1], dtype=tf.float32)
        sliding_result = tf.nn.conv2d(prob_face_1, kernel, [1, 1, 1, 1], padding='SAME')
        sliding_result = tf.divide(sliding_result, tf.square(tf.cast(10, dtype=tf.float32)))

    # gather all boxes
    sliding_result = tf.squeeze(sliding_result, [0, 3])

    graph_mask = tf.subtract(tf.ones(shape=[img_shape[1], img_shape[2]],dtype=tf.float64), 0.5)
    greater = tf.greater(tf.cast(sliding_result, dtype=tf.float64), graph_mask)

    g_bb_xy = tf.where(greater)
    g_bb_xy_left_upper = tf.subtract(g_bb_xy, tf.cast(tf.divide(10, 2), dtype=tf.int64))
    g_bb_xy_right_lower = tf.add(g_bb_xy, tf.cast(tf.divide(10, 2), dtype=tf.int64))
