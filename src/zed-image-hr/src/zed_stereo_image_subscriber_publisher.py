...
class image_From_ZED:
	...
	def __init__(self):
		...
		self.imageLeftResult_pub = rospy.Publisher('/preserve_network/left/low_image', Image, queue_size=10)
		...
		try:
			...
			print(e)
	        	...
	        	with img_buffer_right_lock:
		        	img_buffer_right.push(cv_image.flatten().astype('float32'))
		...
		try:
			...
			print(e)
		        ...
		        with img_buffer_left_lock:
			        img_buffer_left.push(cv_image.flatten().astype('float32'))
...
def imageResultPrint():
	while print_thread_running:
                print("In imageResultPrint, Num of Right Data: ", img_buffer_right.nb_data/IMG_SIZE_ORIGIN)
                print("In imageResultPrint, Num of Left Data: ", img_buffer_left.nb_data/IMG_SIZE_ORIGIN)
